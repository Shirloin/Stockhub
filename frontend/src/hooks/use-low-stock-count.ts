    ...
    useEffect(() => {
        ...
        const startStream = async () => {
            try {
                ...
                const call = client.watchLowStockCount({}, {
                ...
                if (isAbortError) {
                    if (isMounted.current && !abortController.signal.aborted) {
                        ...
                        console.log("gRPC low stock count stream closed, will attempt to reconnect...");
                        setTimeout(() => {
                            if (isMounted.current && !abortController.signal.aborted) {
                                startStream();
                            }
                ...
                if (isMounted.current) {
                    console.error("gRPC low stock count stream error:", err);
