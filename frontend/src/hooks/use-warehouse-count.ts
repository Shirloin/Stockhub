    ...
    useEffect(() => {
        ...
        const startStream = async () => {
            try {
                ...
                const call = client.watchWarehouseCount({}, {
                ...
                if (isAbortError) {
                    if (isMounted.current && !abortController.signal.aborted) {
                        ...
                        console.log("gRPC warehouse count stream closed, will attempt to reconnect...");
                        setTimeout(() => {
                            if (isMounted.current && !abortController.signal.aborted) {
                                startStream();
                            }
                ...
                if (isMounted.current) {
                    console.error("gRPC warehouse count stream error:", err);
