    ...
    useEffect(() => {
        ...
        const startStream = async () => {
            try {
                ...
                const call = client.watchOutOfStockCount({}, {
                ...
                if (isAbortError) {
                    if (isMounted.current && !abortController.signal.aborted) {
                        ...
                        console.log("gRPC out of stock count stream closed, will attempt to reconnect...");
                        setTimeout(() => {
                            if (isMounted.current && !abortController.signal.aborted) {
                                startStream();
                            }
                ...
                if (isMounted.current) {
                    console.error("gRPC out of stock count stream error:", err);
