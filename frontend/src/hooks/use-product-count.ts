    ...
    useEffect(() => {
        ...
        const startStream = async () => {
            try {
                ...
                const call = client.watchProductCount({}, {
                ...
                if (isAbortError) {
                    if (isMounted.current && !abortController.signal.aborted) {
                        ...
                        console.log("gRPC product count stream closed, will attempt to reconnect...");
                        setTimeout(() => {
                            if (isMounted.current && !abortController.signal.aborted) {
                                startStream();
                            }
                ...
                if (isMounted.current) {
                    console.error("gRPC product count stream error:", err);
